...
package org.radarbase.datadashboard.backend.config
...
data class DashboardServiceConfig(
    ...
    fun withEnv(): DashboardServiceConfig = copy(
        ...
        advertisedBaseUri = System.getenv("DATA_DASHBOARD_ADVERTISED_BASE_URI")?.let { URI.create(it) }
            ?: advertisedBaseUri,
        ...
        tokenExpiryTimeInMinutes = System.getenv("DATA_DASHBOARD_TOKEN_EXPIRY_TIME_IN_MINUTES")?.toLong()
            ?: tokenExpiryTimeInMinutes,
        persistentTokenExpiryInMin = System.getenv("DATA_DASHBOARD_PERSISTENT_TOKEN_EXPIRY_IN_MIN")?.toLong()
            ?: persistentTokenExpiryInMin,
    )
}
